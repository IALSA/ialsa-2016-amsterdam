...
ds
# if(is.na(age_death) & !is.na(age_age_visit)

# select_obs <- !is.na(ds$age_death) & is.na(ds$age_at_visit)
# ds[select_obs,"age"] <- ds[select_obs, "age_at_visit"]   

for(i in 1:nrow(ds)){ 
  
  if( is.na(ds[i,"age_at_visit"]) ){
    ds[i, "age"]  <- ds[i,"age_death"]
  }else{
    ds[i,"age"] <- ds[i,"age_at_visit"]
  }
} 
ds %>% dplyr::select(id,fu_point, age_death, age_at_visit, age, state)
ds <- ds %>%  dplyr::select(id, fu_point, age, state)
...
selected_covariates <- dto[["unitData"]] %>% 
  dplyr::select_("id","fu_year", "age_death","age_at_visit",
                 ...
                 "ldai_bl" # Lifetime daily alcohol intake -baseline
                 ) #%>% 
  # dplyr::filter(id %in% ids) #%>% 
  # dplyr::rename(fu_point = fu_year)
...
saveRDS(d, file="./data/unshared/derived/ms-dementia-3-states.rds", compress="xz")
