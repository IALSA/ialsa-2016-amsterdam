...
# ---- declare-globals ---------------------------------------------------------
pathSaveFolder <- "./data/shared/derived/models/model-d/"
digits = 2
...
estimate_multistate("mD1", ds, Q_crude, E, qnames,cov_names = "age")
(Q_crude <- get_crude_Q(ds, Q, "age + male"))
estimate_multistate("mD2", ds, Q_crude, E, qnames,cov_names = "age + male")
(Q_crude <- get_crude_Q(ds, Q, "age + male + educat"))
estimate_multistate("mD3", ds, Q_crude, E, qnames,cov_names = "age + male + educat")
(Q_crude <- get_crude_Q(ds, Q, "age + male + edu"))
estimate_multistate("mD4", ds, Q_crude, E, qnames,cov_names = "age + male + edu")

 
# ---- assemble-msm-models ------------------------
# assemble the list object with the results of msm estimation
models <- list()
models[["age"]][["msm"]]    <- readRDS(paste0(pathSaveFolder,'mD1.rds'))
models[["male"]][["msm"]]   <- readRDS(paste0(pathSaveFolder,'mD2.rds'))
models[["educat"]][["msm"]] <- readRDS(paste0(pathSaveFolder,'mD3.rds'))
