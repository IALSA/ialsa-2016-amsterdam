...
pathSaveFolder <- "./data/shared/derived/models/model-b-mod-2/"
models <- list()
models[["ses"]][["msm"]] <- readRDS(paste0(pathSaveFolder,'mB_mod2_3.rds'))
# models[["ses"]][["msm"]] <- readRDS(paste0(pathSaveFolder,'mB_mod2_4.rds'))
# models[["educat"]][["msm"]] <- readRDS(paste0(pathSaveFolder,'mB_mod2_2.rds'))

# model <- models[["educat"]][["msm"]]
model <- models[["ses"]][["msm"]]
...
le_spread <- function(
  ...
  covar_list <- list(
    ...
    sescat = ds_levles[condition_n, "sescat"]
  )
...
covar_list = list(age=age_min, age_bl=age_bl, male=male, educat=educat, sescat=sescat)
...
saveRDS(les, paste0(pathSaveFolder,"models.rds"))
...
for(model_ in names(models) ){
  ...
  if(model_=="sescat"){covar_list = list(age=age_min, male=male, educat=educat, sescat = sescat)}
  # compute LE
